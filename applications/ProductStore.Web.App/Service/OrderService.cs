...
internal class OrderService : IOrderService
{
    ...
    public async Task<(bool hasValue, OrderModel model)> TryGetModelAsync(CancellationToken cancellationToken)
    {
        var (hasValue, order) = await TryGetOrderAsync(cancellationToken);
        if (hasValue)
    ...
    private async Task<(bool hasValue, Order order)> TryGetOrderAsync(CancellationToken cancellationToken = default)
    {
